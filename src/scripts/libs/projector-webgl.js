...
export default class ProjectorWebGL {
  ...
  rescale(scales, lastScale, projectorSize, settings) {
    try {
      const scalesLengthUnchanged = this.scales?.length === scales.length
      ...
      this.updateCtx(scalesLengthUnchanged)
  ...
  updateCtx(scalesLengthUnchanged = false) {
    if(!this.fragmentShader || !scalesLengthUnchanged) {
    ...
    this.ctx.uniform2fv(this.fScalesLoc, fScales);

    const fScalesMinus = new Float32Array(this.scales.map(({ x, y }) => [((x - 1) / 2), ((y - 1) / 2)]).flat());
    this.ctx.uniform2fv(this.fScalesMinusLoc, fScalesMinus);
