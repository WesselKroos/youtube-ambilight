...
const workerCode = function () {
  ...
  function getAverageColor(imageLines, channels) {
    ...
    const bottomOffsetIndex =
      imageLines[0].data.length - channels - topOffsetIndex;
    ...
    for (const imageLine of imageLines) {
      ...
      i =
        topOffsetIndex +
        (Math.round(edgeOffset * 2 * Math.random()) - edgeOffset) * channels;
  ...
  function detectEdges(imageLines, channels, color) {
    ...
    for (const imageLine of imageLines) {
      ...
      for (
        ...
        const brightnessDeviation = Math.abs(
          data[i] + data[i + 1] + data[i + 2] - (color[0] + color[1] + color[2])
        );
        if (
          // Above the top limit
          i < middleIndex - channels &&
          // Within the color and brightness deviation
          (colorDeviation <= maxColorDeviation ||
            brightnessDeviation <= maxBrightnessDeviation) &&
          colorDeviation + brightnessDeviation <=
            maxColorAndBrightnessDeviationSum
        )
          continue;
  ...
  function getExceedsDeviationLimit(
    ...
    const threshold =
      edges.length * (1 - (allowedAnomaliesPercentage - 10) / 100);
    ...
    const nonDeviatingTopEdges = topEdges
      .filter((p) => !p.deviatesTop)
      .map((e) => e.yIndex);
    const maxTopDeviation = Math.abs(
      Math.max(...nonDeviatingTopEdges) - Math.min(...nonDeviatingTopEdges)
    );
    ...
    const nonDeviatingBottomEdges = topEdges
      .filter((p) => !p.deviatesBottom)
      .map((e) => e.yIndex);
    const maxBottomDeviation = Math.abs(
      Math.max(...nonDeviatingBottomEdges) -
        Math.min(...nonDeviatingBottomEdges)
    );
  ...
  function getPercentage(
    ...
    if (exceedsDeviationLimit) {
      ...
      size = Math.max(
        ...edges.filter((p) => !p.deviates).map((point) => point.yIndex)
      );
      if (size < minSize) {
        size = 0;
      } else {
  ...
  try {
    const workerDetectBarSize = async (
      ...
      const { topEdges, bottomEdges } = detectEdges(
        imageLines,
        channels,
        color
      );
...
export default class BarDetection {
  ...
  detect = (
    ...
    requestIdleCallback(
      async () => await this.idleHandler(run),
      { timeout: 1 },
      true
    );
  };
  ...
  averagePercentage(
    ...
    percentage = Object.keys(percentagesOccurrence).reduce((a, b) =>
    ...
    try {
      ...
      const onMessagePromise = new Promise(
        function onMessagePromise(resolve, reject) {
          ...
          this.worker.onmessage = (e) => {
            try {
              if (this.run !== run || e.data.id !== this.workerMessageId) {
              ...
              const horizontalPercentage = this.averagePercentage(
                horizontalBarSizeInfo.percentage,
                currentHorizontalPercentage || 0,
                this.history.horizontal,
                averageHistorySize
              );
              const verticalPercentage = this.averagePercentage(
              ...
              if (
                ...
                (verticalBarSizeInfo.percentage !== undefined &&
                  Math.abs(
                    verticalBarSizeInfo.percentage - currentVerticalPercentage
                  ) > 0.5)
              ) {
                const barsChanged =
                  (horizontalPercentage !== undefined &&
      ...
      const minThrottle =
        !this.lastChange || this.lastChange + 15000 < now
          ? 1000
